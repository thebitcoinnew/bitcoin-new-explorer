...
export default class DiscoveredPeers extends React.Component {
  ...
  sortedPeers() {
    return this.filteredPeers().sort((a, b) => {
      if (!a.data.btcnewNodeName) return 1;
      if (!b.data.btcnewNodeName) return -1;
      if (a.data.btcnewNodeName.toLowerCase() < b.data.btcnewNodeName.toLowerCase())
        return -1;
      if (a.data.btcnewNodeName.toLowerCase() > b.data.btcnewNodeName.toLowerCase())
        return 1;
      return 0;
    ...
    <tr className={statusClass}>
      <td>
        <a href={rootUrl} className={statusClass} target="_blank">
          <OptionalField value={data.btcnewNodeName} />
        </a>
      </td>
      <td>
      ...
      <td>
        <AccountLink account={data.btcnewNodeAccount} short />
      </td>
    </tr>
  );
};
