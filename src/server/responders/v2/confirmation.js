...
export default function(app, btcnew) {
  app.get("/confirmation/active", async (req, res, next) => {
    try {
      const data = await redisFetch("confirmation_active", 2, async () => {
        const hashes = (await btcnew.rpc("confirmation_active")).confirmations;
        if (hashes === "") return [];

        let hydratedData = [];
        for (let i = 0; i < hashes.length; i++) {
          const info = await btcnew.rpc("confirmation_info", {
            root: hashes[i]
          });
  ...
  app.get("/confirmation/info/:hash", async (req, res, next) => {
    try {
      const data = await btcnew.rpc("confirmation_info", {
  ...
  app.get("/confirmation/history", async (req, res, next) => {
    try {
      const data = await redisFetch("confirmation_history", 10, async () => {
        const resp = await btcnew.rpc("confirmation_history");
