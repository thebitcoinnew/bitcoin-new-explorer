  ...
  app.get("/network/active_difficulty", async (req, res, next) => {
    try {
      const data = await redisFetch(
        ...
        10,
        async () => {
          return await btcnew.rpc("active_difficulty", { include_trend: true });
        }
      );
  ...
  app.get("/network/peers", async (req, res, next) => {
    try {
      const data = await redisFetch("v2/network/peers", 300, async () => {
        const quorumPeers = (await btcnew.rpc("confirmation_quorum", {
          peer_details: true
        })).peers;

        const allPeers = (await btcnew.rpc("peers", { peer_details: true }))
          .peers;
