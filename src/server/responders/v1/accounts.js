  ...
  app.get("/account/:account", async (req, res, next) => {
    ...
    try {
      const account = await redisFetch(
        ...
        10,
        async () => {
          const account = await btcnew.rpc("account_info", {
      ...
      const weight = await redisFetch(
        ...
        async () => {
          return Currency.fromRaw(
            await btcnew.accounts.weight(req.params.account)
          );
        }
      );
  ...
  app.get("/account/:account/delegators", async (req, res, next) => {
    ...
    try {
      const delegators = await redisFetch(
        ...
        300,
        async () => {
          const resp = await btcnew.rpc("delegators", {
            account: req.params.account
          });
          return _.fromPairs(
  ...
  app.get("/account/:account/history", async (req, res, next) => {
    ...
    try {
      const history = await redisFetch(
        ...
        10,
        async () => {
          // const resp = await btcnew.accounts.history(req.params.account, 20);
          const resp = (await btcnew.rpc("account_history", {
            account: req.params.account,
            count: 50,
            raw: "true",
            head: req.query.head
  ...
  app.get("/account/:account/pending", async (req, res, next) => {
    ...
    try {
      const data = await redisFetch(
        ...
        10,
        async () => {
          const resp = await btcnew.rpc("accounts_pending", {
