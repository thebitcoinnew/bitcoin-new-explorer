...
import { Nano as BitcoinNew } from "nanode";
...
async function calculateAccountList() {
  ...
  const frontierCount = (await btcnew.rpc("frontier_count")).count;
  const data = (await btcnew.rpc("frontiers", {
    account: `${
  ...
  for (let i = 0; i < accountChunks.length; i++) {
    const resp = await btcnew.accounts.balances(accountChunks[i]);
...
function updateSortedAccounts(accountsWithBalance, accountsToRemove) {
  const sortedKey = `btcnew-control-panel/${config.redisNamespace ||
...
function updateAllAccounts(accountsWithBalance, accountsToRemove) {
  const unsortedKey = `btcnew-control-panel/${config.redisNamespace ||
    "default"}/allAccounts`;
