# Bitcoin New Explorer

A lightweight web UI for viewing realtime information about your Bitcoin New node and exploring the Bitcoin New network.

## What is Bitcoin New?

Bitcoin New's goal is to become "a global currency with instantaneous transactions and zero fees over a secure, decentralized network." More information is available over on the official [Bitcoin New repository](https://github.com/thebitcoinnew/bitcoin-new-node).

## Installation

First, clone this repository onto the server where you intend to host the site. It doesn't have to be the same server as the Bitcoin New node, but it certainly can be if you want to.
...
The server is responsible for proxying requests between the site and your Bitcoin New node. You should never expose your Bitcoin New node RPC to the public, which is why we have a server that exposes only certain endpoints. It also does a little processing on the raw response from the Bitcoin New RPC and caches responses with Redis.
...
The [websocket server](https://github.com/thebitcoinnew/bitcoin-web-wallet-ws) is optional, but you're welcome to use the hosted websocket server that's set as the default in the config. Depending on the sync status of your node, you may receive blocks from the websocket server before your node confirms them, which is why hosting one yourself is ideal. Remove the config entry to disable the websocket altogether.

## Development

To run Bitcoin New Explorer in development mode, simply run `yarn start`. This will start both the API server and the webpack development server for the front-end. This does not start any of the reoccuring jobs, but can you run those manually if you need the data they provide (see below).
...
server {
  ...
  index index.html;
  server_name btcnew.yourdomain.com;
...
- `server.peers.js` - reoccurring job for discovering and fetching data from other Bitcoin New monitors
...
Bitcoin New Explorer aims to be available in as many languages as possible. If you would like to contribute translations, please see the instructions below and send a Pull Request when ready.
...
English is the fallback language for Bitcoin New Explorer if a particular translation is not present.

To contribute to Bitcoin New Explorer's translations, please email meltingice (see Github profile) to get an invite to our [POEditor](https://poeditor.com/) project. While discouraged, we will accept pull requests with translation updates as well.
